...
OUT_PATH   = "drf_nested_predictions.npy"
...
def main():
    ...
    for o, (tr_out_idx, te_out_idx) in enumerate(outer_kf.split(X_df, strat_outer), 1):
        ...
        prep_out = fit_preprocessor(X_df.iloc[tr_out_idx], ohe_drop_first=False, include_is_working=True)
        X_tr_out = prep_out.transform(X_df.iloc[tr_out_idx]).to_numpy(dtype=float)
        y_tr_out = y[tr_out_idx]
        X_te_out = prep_out.transform(X_df.iloc[te_out_idx]).to_numpy(dtype=float)
        ...
        for pg in param_grid:
            ...
            for i, (tr_in_idx, va_in_idx) in enumerate(inner_kf.split(np.zeros_like(strat_inner), strat_inner), 1):
                ...
                inner_va_rows = X_df.iloc[tr_out_idx].iloc[va_in_idx]
            ...
            cfg_scores.append((pg, mean_inner))
            print(f"[Outer {o}] Params {pg}  ->  Mean INNER-CRPS: {mean_inner:.6f}")

        # pick best params for this outer fold
        best_pg, best_inner = min(cfg_scores, key=lambda t: t[1])
        outer_choices.append(best_pg)
        print(f"[Outer {o}] Selected params: {best_pg}  (mean INNER-CRPS: {best_inner:.6f})")
        ...
        rf_out = RandomForestRegressor(
            ...
            bootstrap=True,
            n_jobs=-1,
            random_state=RNG_SEED + o
        )
        drf_out = DistributionalForest(rf_out, use_log1p=True, tau=best_pg["tau"]).fit(X_tr_out, y_tr_out)
        rng = np.random.default_rng(RNG_SEED + o*1000)
        samples_te_out = drf_out.sample(X_te_out, n_samples=N_SAMPLES_CV, rng=rng)
        crps_outer = crps_ensemble(y_te_out, samples_te_out).mean()
        outer_scores.append(float(crps_outer))
        print(f"--> [Outer {o}] OUTER-TEST CRPS: {crps_outer:.6f}")
